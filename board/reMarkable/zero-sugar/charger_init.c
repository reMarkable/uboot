...
#define MAX77818_CHG_CNFG_02__CHARGE_CC__FAST_CHARGE_1_5MA  0x1E
...
#define MAX77818_CHG_CNFG_09__CHGIN_ILIM_1500_M_A_MAX       0x2D
...
int max77818_set_fast_charge_current(struct udevice *dev, enum fast_charge_current fsc)
{
	...
	switch(fsc)
	{
	case FASTCHARGE_1_5MA:
		fc_config_value = MAX77818_CHG_CNFG_02__CHARGE_CC__FAST_CHARGE_1_5MA;
		break;
	case FASTCHARGE_2_8MA:
		fc_config_value = MAX77818_CHG_CNFG_02__CHARGE_CC__FAST_CHARGE_2_8MA;
		break;
	default:
...
int max77818_set_pogo_input_current_limit(struct udevice *dev, enum pogo_ilim ilim)
{
	...
	{
	case ILIM_1_5MA:
		ilim_config_value = MAX77818_CHG_CNFG_09__CHGIN_ILIM_1500_M_A_MAX;
		break;
	case ILIM_2_8MA:
		ilim_config_value = MAX77818_CHG_CNFG_09__CHGIN_ILIM_2800_M_A_MAX;
		break;
	default:
		/* Invalid value, just set default 1.5A */
		ilim_config_value = MAX77818_CHG_CNFG_09__CHGIN_ILIM_1500_M_A_MAX;
...
int zs_do_set_fastcharge_current_1500_ma(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int ret;
	...
	ret = max77818_set_fast_charge_current(chDev, FASTCHARGE_1_5MA);
	if (ret) {
...
U_BOOT_CMD(
	max77818_set_fast_charge_current_1500_ma, 1, 1, zs_do_set_fastcharge_current_1500_ma,
...
int zs_do_set_fastcharge_current_2800_ma(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	ret = max77818_set_fast_charge_current(chDev, FASTCHARGE_2_8MA);
	if (ret) {
		printf("Failed to set fast charge current: %d\n", ret);
		return ret;
	}
...
int zs_do_set_pogo_input_current_limit_1500_ma(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	ret = max77818_set_pogo_input_current_limit(chDev, ILIM_1_5MA);
	if (ret) {
...
U_BOOT_CMD(
	max77818_set_pogo_input_current_limit_1500_ma, 1, 1, zs_do_set_pogo_input_current_limit_1500_ma,
...
int zs_do_set_pogo_input_current_limit_2800_ma(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	ret = max77818_set_pogo_input_current_limit(chDev, ILIM_2_8MA);
	if (ret) {
		printf("Failed to set pogo input current limit: %d\n", ret);
		return ret;
	}
...
U_BOOT_CMD(
	max77818_set_pogo_input_current_limit_2800_ma, 1, 1, zs_do_set_pogo_input_current_limit_2800_ma,
